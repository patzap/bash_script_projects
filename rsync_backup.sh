...
if [[ -d ${dir_name} ]]; then 
	...
	if [[ ${num_args} -eq 1 ]]; then
		...
		#rsync -av ${dir_name} pz@192.168.56.10:~/backup_txt/
	...
	#elif [[ ${dir_name} != ${path} ]]; then
		#echo "Error. Please enter an argument when executing command. Example <./rsync_ex2.sh ~/path/of/dir/source>"

	fi

else #default backups that will be executed automatically 
	echo "No argument was given, will proceed to automatic backup..."
	date >> rsync_log.txt
	rsync -avh --exclude=".*" ~/Backup patzap@192.168.2.55 >> rsync_log.txt #Stores backup data on a txt file
	#echo ${log}
